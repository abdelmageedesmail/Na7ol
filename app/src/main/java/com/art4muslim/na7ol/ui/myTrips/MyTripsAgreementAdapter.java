...
public class MyTripsAgreementAdapter extends RecyclerView.Adapter<MyTripsAgreementAdapter.MyHolder> {
    Context context;
    ...
    AddRate addRate;
    EditPriceOffer editPriceOffer;
    PrefrencesStorage storage;

    public MyTripsAgreementAdapter(@NonNull Context context, List<AgreementsResponseModel.ReturnsEntity> returns) {
    ...
    public void setEditPriceOffer(EditPriceOffer editPriceOffer) {
        this.editPriceOffer = editPriceOffer;
    }
...
//                }
                ...
                Intent intent = new Intent(context, AgreementTripDetails.class);
        ...
        myHolder.frOptions.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (list.get(myHolder.getAdapterPosition()).getAgr_status().equals("pending")) {
                    popup.inflate(R.menu.agreement_menu_pending);
                    popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                        @Override
                        public boolean onMenuItemClick(MenuItem item) {
                            switch (item.getItemId()) {
                                case R.id.tvCancel:
                                    if (editPriceOffer != null) {
                                        editPriceOffer.onEditPriceOffer(myHolder.getAdapterPosition(), "cancel");
                                    }
                                    return true;
                                case R.id.tvEditOrder:
                                    if (editPriceOffer != null) {
                                        editPriceOffer.onEditPriceOffer(myHolder.getAdapterPosition(), "edit");
                                    }
                                    return true;
                            }
                            return false;
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            switch (item.getItemId()) {
                                ...
                                case R.id.tvChat:
                                    ...
                                    if (list.get(myHolder.getAdapterPosition()).getAgr_to_id().equals(storage.getId())) {
                                    ...
                                    intent.putExtra("tripId", list.get(myHolder.getAdapterPosition()).getAgr_serv_id());
                                    context.startActivity(intent);
                                    return true;
                                case R.id.tvTerms:
                                    ...
                                    if (storage.getId().equals(list.get(myHolder.getAdapterPosition()).getAgr_from_id())) {
                                        intent1.putExtra("driverId", "" + list.get(myHolder.getAdapterPosition()).getAgr_to_id());
                                    } else {
    ...
    public interface EditPriceOffer {
        void onEditPriceOffer(int pos, String type);
    }
