...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ChatHolder> {
    ...
    public ChatAdapter(@NonNull Context context, List<ChatModel> arrayList) {
...
//        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        if (arrayList.get(i).getFrom().equals(storage.getId())) {
...
//            chatHolder.tvMsg.setGravity(Gravity.START);
            ...
            chatHolder.frChat.setLayoutParams(paramsMsg);
            if (!arrayList.get(i).getToImage().isEmpty()) {
                Picasso.with(context).load(arrayList.get(i).getToImage()).into(chatHolder.ivProfile);
            }
        }
        chatHolder.tvMsg.setText(arrayList.get(i).getMessage());

        if (arrayList.get(i).getFile().isEmpty()) {
            chatHolder.ivImage.setVisibility(View.GONE);
            chatHolder.ivLocation.setVisibility(View.GONE);
            ...
            Picasso.with(context).load(arrayList.get(i).getFile()).into(chatHolder.ivImage);
        }
        if (arrayList.get(i).getLat() != null) {
            if (!arrayList.get(i).getLat().isEmpty()) {
                chatHolder.ivImage.setVisibility(View.GONE);
                chatHolder.ivLocation.setVisibility(View.VISIBLE);
                chatHolder.ivLocation.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        messageLocation = arrayList.get(chatHolder.getAdapterPosition()).getLat() + "," + arrayList.get(chatHolder.getAdapterPosition()).getLon();
//                    String[] split = arrayList.get(chatHolder.getAdapterPosition()).getMsg_location().split(",");
                        String geoUri = "http://maps.google.com/maps?q=loc:" + messageLocation + " (" + ")";
...
//                context.startActivity(intent);
    ...
    public class ChatHolder extends RecyclerView.ViewHolder implements OnMapReadyCallback {
        ...
        FrameLayout frShape;
        ImageView ivImage, ivProfile, ivLocation;
        MapView map_view;
        GoogleMap gmap;
